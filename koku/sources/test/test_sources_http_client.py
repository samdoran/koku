...
class SourcesHTTPClientTest(TestCase):
    ...
    def test_get_cost_management_application_type_id(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                f"{MOCK_URL}/{MOCK_PREFIX}/{ENDPOINT_APPLICATION_TYPES}?filter[name]=/insights/platform/cost-management",
                status_code=200,
        ...
        for test in table:
            with self.subTest(test=test):
                with patch.object(SourcesHTTPClient, "get_cost_management_application_type_id", return_value=2):
                    with requests_mock.mock() as m:
                        m.get(
                            f"{MOCK_URL}/{MOCK_PREFIX}/{ENDPOINT_APPLICATION_TYPES}/{COST_MGMT_APP_TYPE_ID}/sources?filter[id]={self.source_id}",
    ...
    def test_get_aws_credentials_username(self):
        ...
        with requests_mock.mock() as m:
            resource_id = 2
            m.get(
                f"{MOCK_URL}/{MOCK_PREFIX}/{ENDPOINT_AUTHENTICATIONS}?filter[source_id]={self.source_id}&filter[authtype]={auth_type}",
                status_code=200,
    ...
    def test_get_aws_credentials_errors(self):
        ...
        with requests_mock.mock() as m:
            ...
            resource_id = 2
            m.get(
                f"{MOCK_URL}/{MOCK_PREFIX}/{ENDPOINT_AUTHENTICATIONS}?source_id={self.source_id}&authtype={auth_type}",
                status_code=200,
